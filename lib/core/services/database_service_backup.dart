...
class DatabaseService {
  ...
  Future<Database> get database async {
    ...
    // Ensure payments table exists after database initialization
    await _ensurePaymentsTableExists();
  ...
  Future<void> _onCreate(Database db, int version) async {
    try {
      ...
      await db.execute(UserDao.createTableSQL);

      // Create auth_tokens table
      await db.execute(AuthTokenDao.createTableSQL);

      // Create billing_records table
      await db.execute(BillingRecordDao.createTableSQL);
      ...
      await db.execute(PaymentDao.createTableSQL);
      await db.execute(PaymentDao.createTransactionsTableSQL);
  ...
  Future<void> _ensureSubscriptionsTableExists() async {
    try {
      ...
      if (tables.isEmpty) {
        ...
        await db.execute(SubscriptionDao.createTableSQL);
  ...
  Future<void> _ensureSyncMetadataTableExists() async {
    try {
      ...
      if (tables.isEmpty) {
        ...
        await db.execute(SyncMetadataDao.createTableSQL);
  ...
  Future<void> _ensureProductsTableExists() async {
    try {
      ...
      if (tables.isEmpty) {
        await db.execute(ProductDao.createTableSQL);
        await db.execute(ProductDao.createIndexesSQL);
  ...
  Future<void> _ensurePlansTableExists() async {
    try {
      ...
      if (tables.isEmpty) {
        await db.execute(PlanDao.createTableSQL);
        await db.execute(PlanDao.createPlanFeaturesTableSQL);
  ...
  Future<void> _ensurePaymentsTableExists() async {
    try {
      final db = await database;
      final tables = await db.query(
        'sqlite_master',
        where: 'type = ? AND name = ?',
        whereArgs: ['table', 'payments'],
      );
      ...
      if (tables.isEmpty) {
        ...
        await db.execute(PaymentDao.createTransactionsTableSQL);
        _logger.d(
          'Payments and payment_transactions tables created successfully',
        );
      }
    } catch (e) {
      _logger.e('Error ensuring payments table exists: $e');
      rethrow;
    }
  }

  // Ensure tags table exists
  Future<void> _ensureTagsTableExists() async {
    try {
      final db = await database;
      final tables = await db.query(
        'sqlite_master',
      ...
      if (tables.isEmpty) {
        await db.execute(TagsDao.createTableSQL);
        _logger.d('Tags table created successfully');
      }
    } catch (e) {
