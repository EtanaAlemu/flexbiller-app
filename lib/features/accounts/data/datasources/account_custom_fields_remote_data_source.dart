    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['success'] == true && responseData['data'] != null) {
          ...
          throw ServerException(
            responseData['message'] ?? 'Failed to fetch custom fields',
          );
        }
      } else {
        throw ServerException(
          'Failed to fetch custom fields: ${response.statusCode}',
        );
      }
    } on DioException catch (e) {
      if (e.response?.statusCode == 401) {
        throw AuthException('Unauthorized access to custom fields');
      } else if (e.response?.statusCode == 403) {
        throw AuthException(
          'Forbidden: Insufficient permissions to access custom fields',
        );
      } else if (e.response?.statusCode == 404) {
        ...
        throw ServerException('Failed to fetch custom fields: ${e.message}');
      }
    } catch (e) {
